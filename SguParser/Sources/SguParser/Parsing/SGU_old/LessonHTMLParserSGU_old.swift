...
public struct LessonHTMLParserSGU_old: LessonHTMLParser {
    public init() {
        
    }

    public func getScheduleFromSource(source html: String) throws -> [LessonDTO] {
        do {
    ...
    public func getGroupScheduleFromSource(source html: String, groupNumber: String, departmentCode: String) throws -> GroupScheduleDTO {
        do {
            let lessonsByDays = try getLessonsFromSource(source: html)
            return GroupScheduleDTO(groupNumber: groupNumber, departmentCode: departmentCode, lessonsByDays: lessonsByDays, lastUpdated: "-")
        } catch {
            throw NetworkError.htmlParserError
        }
    }
